...
class SURF:
    ...
    def show_feature_points(self):
        for image_index in range(len(self.gray_image_array) - 1):
            ...
            sub_image1_key_points = self.surf.detect(sub_image1, None)
            sub_image2_key_points = self.surf.detect(sub_image2, None)
            ...
            cv2.imshow('SURF FEATURES ' + str(image_index) + "-" + str(image_index + 1), np.concatenate((sub_image1, sub_image2), axis=1))
            cv2.waitKey(0)
    ...
    def show_feature_matches(self):
        for image_index in range(len(self.gray_image_array) - 1):
            ...
            sub_image1_key_points, sub_image1_descriptor = self.surf.detectAndCompute(sub_image1, None)
            sub_image2_key_points, sub_image2_descriptor = self.surf.detectAndCompute(sub_image2, None)
            ...
            cv2.imshow('SURF FEATURE MATCHES ' + str(image_index) + "-" + str(image_index + 1), result_image)
            cv2.waitKey(0)
    ...
    def stitch_images(self):
        ...
        for image_index in range(self.number_of_images - 1, 0, -1):
            ...
            left_image_kps, left_image_descs = self.surf.detectAndCompute(left_image, None)
            right_image_kps, right_image_descs = self.surf.detectAndCompute(right_image, None)
        ...
        cv2.imshow("SURF-Stitched Image", stitched_image)
        cv2.waitKey(0)
